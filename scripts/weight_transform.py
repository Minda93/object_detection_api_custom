...
def save_layer_param(cfg, ckpt, save_path, depth_wise, layer_names):
    ...
    if(save_path is not None):
        ...
        with open(save_path,"wb") as outfile:
            ...
            #     if(len(weight_dict[name].shape) == 4):
            #         if((count in set(depth_wise))):
            #             shape = weight_dict[name].shape
            #             weight_dict[name] = np.transpose(weight_dict[name],(2,3,0,1))
            #             print(count, 0, name, shape, weight_dict[name].shape)
            #         else:
            #             shape = weight_dict[name].shape
            #             weight_dict[name] = np.transpose(weight_dict[name],(2,3,1,0))
            #             print(count, 1, name, shape, weight_dict[name].shape)
            #     else:
            #         print(count, 2, name, weight_dict[name].shape)
            #     count += 1
...
def main():
    ...
    parser.add_argument("--save-path", type=str, default=None, help="Trained weights.")
    ...
    # read layer name for tensorflow version (manual)
    # layer_names = read_layer_name_from_file("/mnt/storage/pytorch_model/models/mobilenet_fpn6_mixconv_512_bdd/layer_name_custom.txt")
    layer_names = read_layer_name_from_file(args.layer_name_torch)
    # print(layer_names)
    
    # read depthwise from tensorflow model
    # depth_wise = get_depth_wise("/mnt/storage/pytorch_model/models/mobilenet_fpn6_mixconv_512_bdd/layer_name_tf.txt")
    depth_wise = get_depth_wise(args.layer_name_tf)
    print(depth_wise, len(depth_wise))
