...
def main():
  ...
  graph_def=get_graph_def_from_file(model_dir+"/"+graph_path)
  for node in graph_def.node:
    if node.op=='Placeholder':
      print("##### model - Input Node Name #####", node.name) # this will be the input node
      input_names=node.name
  
  # Optimizing the graph via TensorFlow library
  transforms = []
  optimize_graph(model_dir, graph_path, transforms, input_names, output_node_names, outname='optimized_model.pb')
  
  # convert this to a s TF Serving compatible mode
  shutil.rmtree(model_dir+'/0', ignore_errors=True)
  convert_graph_def_to_saved_model(model_dir+'/save_model',
                                   model_dir+'/optimized_model.pb', input_names, outputs)
